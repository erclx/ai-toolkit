...
- Two verbs cover the structured edits the board takes. `aitk tasks pull-request <number> --plan <slug>` records a pull request on the task it closes, and `aitk tasks outcome --plan <slug> --close <n>` marks an outcome shipped. Both accept `--json` and refuse rather than guess, so a caller branches on `reason`.
