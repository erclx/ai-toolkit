  ...
  worktrees
    ...
    .addHelpText(
      ...
      [
        ...
        'applies: "session" when a live session holds the directory. `claude',
        'rm <id>` is meant to remove it and its worktree together, but its',
        'own report is not reliable, so run this command again afterward to',
        'see what happened. "worktree" is the shape once the session has',
        'ended, where `canon worktrees reclaim` is the pair of a remove and',
        'a branch delete.',
        '',
        'Examples:',
