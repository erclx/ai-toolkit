 ...
 * target's own root file. A `See` or a `per`, then anything up to a period or
 * a backtick, then ` in ` and a backticked `CLAUDE.md`. The two verbs are what
 * separate a citation from a mention: dropping them returns five extra lines
 * that name the file without pointing into it, and the `[^.`]` class stops the
 * match at a sentence boundary so a body discussing the file two sentences
 * earlier does not fail.
 *
