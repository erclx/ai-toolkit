...
- Standards and snippets author at project-root `standards/` and `snippets/`, but install into a target under `.claude/standards/` and `.claude/snippets/`. The toolkit commits its own consumed copy under `.claude/`, regenerated by `bun run check`. Rules, skills, prompts, and seeds reference `.claude/standards/X.md` so paths resolve identically in the toolkit and target installs. This mirrors the `governance/rules/` to `.claude/rules/` split. `$CLAUDE_PLUGIN_ROOT` and `$CLAUDE_PROJECT_DIR` are empty in model Bash.
