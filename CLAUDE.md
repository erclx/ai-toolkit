...
### Authoring and install layout

- Toolkit-internal content (runbooks, dev helpers) is authored under `internal/`, never inside an installable surface. Location is what enforces the boundary, because a filter in code covers only the entry points that run code, and the plugin reaches `standards/` and `snippets/` through symlinks an installer dereferences with nothing in the path to filter.
- `scripts/core/check-plugin-boundary.sh` fails on any file the plugin ships that resolves under `internal/`.
- Standards and snippets author at project-root `standards/` and `snippets/`. Snippets install into a target under `.claude/snippets/`. Standards install nowhere, so a target reads one through `aitk standards <name>`. The toolkit commits its own consumed copy of both under `.claude/`, regenerated by `bun run check`. Rules, skills, and seeds reference `.claude/standards/X.md`, which resolves here against that copy.
- The same split runs from `governance/rules/` to `.claude/rules/`, and `internal/` follows it to `.claude/internal/`, installing into no target except `internal/rules/`, which lands in `.claude/rules/`.
