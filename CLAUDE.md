...
- Never delete a memory entry. `claude-memory-review` moves a retired one to `.claude/.tmp/memory-archive/`, since the folder is gitignored and a wrong call has no undo.
