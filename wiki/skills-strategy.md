...
**Workflow skills** live in `claude/skills/` and wrap how this toolkit operates: groundwork, planning, review, shipping, debugging, git, governance install. They are thin, opinionated, and specific to the author's process. The current set covering `aitk:claude-groundwork`, `aitk:claude-feature`, `aitk:claude-review`, `aitk:git-ship`, and similar all belong here.
...
Five toolkit skills were compared against community counterparts. All five are kept. One was lightly enhanced.

- `aitk:systematic-debugging` vs `obra/superpowers/systematic-debugging`. Same methodology. Ours is 71 lines to their 296. Our version captures the four phases, circuit breaker, and red flags in a prose-tight form that matches toolkit conventions. No borrow.
...
- `aitk:claude-feature` vs `obra/superpowers` planning skills: `brainstorming`, `writing-plans`, and `executing-plans`. Different slots. Ours reads the full `.claude/` doc set and produces a structured plan at a coarser grain than superpowers' task-atomized `writing-plans`. Plan mode and Ultraplan are positioned separately in `.claude/context/claude-plugin.md`. No borrow: the approval gate between plan and implement already covers the clarification case that `brainstorming` handles upfront.
- `aitk:claude-groundwork` vs `obra/superpowers/brainstorming`. Closest external analogue, different output. `brainstorming` is an upfront clarification conversation that feeds straight into a plan, so its product is a better-specified plan. Groundwork produces a durable numbered folder of measurements and rejected options that can legitimately conclude in doing nothing, and it runs before a plan is warranted rather than while one is being written. No borrow.
