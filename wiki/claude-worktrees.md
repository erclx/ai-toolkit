...
**Merge order.** Merge the branch with the smallest shared-hotspot footprint first. If any branch touches `CLAUDE.md`, `docs/claude.md`, or a regenerated `index.md`, that branch merges last. Its siblings rebase on the new `main` once it lands. See [Fan-out rules](#fan-out-rules-for-this-toolkit) for the hotspot list.
