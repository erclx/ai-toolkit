...
stage_setup() {
  ...
  log_info "open    : drifted workspace, no .claude/.tmp/ folder yet"
...
EOF
    ...
    log_info "         Two or more approaches are live and no .claude/.tmp/ folder exists"
    log_info "Action:  /claude-groundwork tooling config drift across the three packages"
    log_info "Expect:  qualifying test passes, folder created at .claude/.tmp/<slug>/"
  ...
  "resume")
    ...
    mkdir -p .claude/.tmp/tooling-drift
    cat <<'EOF' >.claude/.tmp/tooling-drift/README.md
# Tooling drift
...
EOF
    ...
    cat <<'EOF' >.claude/.tmp/tooling-drift/01-current-state.md
# Current state
...
EOF
    ...
    log_info "Context: .claude/.tmp/tooling-drift/ exists with README.md and 01-current-state.md"
    ...
    log_info "Action:  /claude-groundwork tooling drift"
...
EOF
    ...
    log_info "Action:  /claude-groundwork adding a --help flag"
    log_info "Expect:  skill declines and points at /claude-feature"
    log_info "         NO folder created under .claude/.tmp/"
