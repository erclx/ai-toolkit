...
stage_setup() {
  ...
  "create")
    ...
    log_info ""
    log_info "Action:  /aitk:claude-tasks create a task for the CSV export endpoint,"
    ...
    log_info "         Check it with: aitk sandbox check claude:tasks create"
  ...
  "archive")
    ...
    log_info "  Plan: line already points into .claude/.tmp/plans-archive/, so the"
    ...
    log_info ""
    log_info "Action:  /aitk:claude-tasks archive v01.0-rate-limit"
    log_info "Expect:  declared in fixtures/claude/tasks/archive/expect.toml"
    log_info "         Check it with: aitk sandbox check claude:tasks archive"
    log_info "         The task moved under .claude/.tmp/task-archive/, its ordering"
