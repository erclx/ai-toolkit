...
- Every standard that governs a file path carries a rule routing to it, so an edit loads the standard without the matching skill being invoked. `595-tooling-reference` is toolkit-local and lives only in `.claude/rules/claude/`, because `standards/aitk/tooling-reference.md` governs a surface a target never authors and shipping the route would contradict the decision that put the standard in `standards/aitk/`.
