...
The flat form of that claim did not hold on 2026-08-31. One worker session reached `claude-autoship` through the `Skill` tool and ran the whole chain, while another was refused at the same call on another branch the same day, and both launch prompts named the skill as a slash command. Nothing established what separated them. So read the paragraph above as behavior a chain may not depend on rather than behavior it can predict, and note that the instruction following from it is unchanged either way, since the launch prompt's own slash commands are the route no dispatch has been refused at. `claude/skills/claude-orchestrate/references/orchestrator-dispatch.md` states that where a dispatcher writes one.
