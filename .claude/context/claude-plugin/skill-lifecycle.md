...
A peer session reporting on the same day named the cause directly: a slash command expands as a user invocation only at position zero of a prompt, so a later one in the same text, chained after a first, reaches the session as prose with the `Skill` tool as its only remaining route, which the flag then refuses. That resolves what the paragraph above left open: a launch leading with the flagged body's own command reaches position zero and runs, and a launch that puts anything ahead of it does not.
