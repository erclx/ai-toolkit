...
A dispatch run on 2026-09-02 tried the untested candidate above: a launch prompt leading with `/canon:claude-autoship <plan>`, carrying only a conditional fallback naming `claude-worktree` for a worktree already in place. The worker's own report, read from inside the session it launched, said the launch reached the flagged body as an expanded user invocation in a `<command-message>` block rather than through the `Skill` tool, so no refusal was ever in play there, and that chain's own Step 0 entered the worktree through the unflagged `claude-worktree`, so the fallback never fired either. One success does not settle the flag's inconsistency, because this run never exercised the `Skill` tool route the flag gates, and the flag's answer through the `Skill` tool is already measured as inconsistent rather than as a hard failure. Read this as one further data point on the position-zero expansion mechanism rather than as proof it closes the disable-model-invocation inconsistency.

A second dispatch the same day met the inverse case and explains rather than only extends it. That launch led with `/canon:claude-worktree <name>` and chained a second command, `/canon:claude-autoship <plan>`, later in the same prompt. The leading command took position zero and ran as an invocation, and the second reached the session as prose, so the `Skill` tool call it fell back to was refused.
