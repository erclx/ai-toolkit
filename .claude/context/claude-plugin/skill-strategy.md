...
Two family verdicts were re-measured against premises that had moved since they were first taken, and a third stayed untouched because its premise had not. The `migration-*` admission rule holds as a rule, but its membership has drifted: `migration-standards` relocates a root `snippets/` folder alone, and the standards corpus now installs into no project at all, so the family carries no member for the move running by hand today across three target projects, dropping an installed `.claude/standards/` tree and repointing every citation at `aitk standards <name>`. A backlog task files that gap rather than closing it here. The `toolkit-cli` boundary moved too, since its subject is what `aitk` sync and install commands overwrite and the standards install channel it used to document has since closed, but the skill's own overwrite contract already reads `Standards | none | Nothing installs`, so nothing there needed a fix. `setup-init` still names its four siblings under `Out of scope` and still invokes `setup-verify` as chain step 4, so the setup-chain admission holds unchanged.
