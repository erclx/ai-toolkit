...
| `decision-escalate`    | No. `005-behavior.md` states the question surface for every session, and this skill is what batches several open calls into one turn                                               | Yes, a cap of four in one turn, a stop that forbids acting on the recommendation, and the plan-standard rule rewriting `- Suggested:` while leaving `- Answer:` blank                                | 27 typed invocations across 3 projects, the highest typed reading of the four, and 4 tool calls in 3 transcripts against a flag that bars routing                 |
