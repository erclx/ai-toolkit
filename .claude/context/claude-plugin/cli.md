---
title: CLI
description: The aitk claude command surface and what each verb writes into a target project
---
...
The hook straddles the two halves and reads only one of them. Its `standard=` path is `prose.md`, where every `- Do not use` bullet sits, and the two character bans stay hardcoded because their rule text moved to `markdown.md` when the standard split. Nothing checks the pair, so a `Do not use` bullet added to `markdown.md` is parsed by nothing and the file still passes every gate. The coupling is named in both standards and in the hook's own comment until the rules-as-a-layer work gives it a real home.
