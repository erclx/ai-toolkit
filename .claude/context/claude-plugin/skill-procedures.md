...
Entering from a branch that is already conventional now stops on the rename collision. The name derives back onto the branch the session came from, git refuses a second branch under it, and the type default cannot invent a distinct one. The bare-name rename this replaces carried the same collision to `git-branch`, which has no check of its own, so the stop moves the failure to the step that can still describe it.
