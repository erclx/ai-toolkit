...
Entry from the main worktree is the case `claude-autoship` cannot reach, and the cause is the order its own body puts two sections in. Its guards derive the slug from the current branch and stop when `.claude/plans/feature-<slug>.md` is absent, while Step 0 is what enters the worktree that gives the branch its name. On `main` the slug resolves to `main`, no plan is filed under that name, and the guard stops the chain before Step 0 runs. `claude-worktree` anticipates the same entry and handles it, tiering through the single plan file, the multiple-plan question, and an inference from session context, so the two bodies disagree about whether starting from `main` is supported. Measured 2026-08-20 running the chain against a task file from `main`, where the plan was resolved from the task's subject rather than from the branch.
