...
A census closed that set. Measured across the 138 commits between the map shipping and `190af80b`, the uncovered surfaces fell from 28 to 8 and the commits carrying no label from 54 to 53, every one of the 53 a release. Eleven rows carry 36 prefixes where nine carried twelve. The remaining 8 are recorded in the file itself rather than covered, being four paths a release moves and four a command rewrites, since a domain row over either gives a mechanical edit a subject it does not have.
