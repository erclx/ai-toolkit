...
That test is what bounds the rule. The two coverage sweeps in `claude-docs` moved because a project with no wireframe folder and a diff adding no source signal skips both outright, and the rebase machinery in `claude-address-review` moved because a branch that still merges never reaches it. A reply format the same skill writes on every run stayed, having failed the test.
...
A reference two skills read gets a copy under each, generated from the single source. Pointing one skill at a sibling skill's folder is banned outright: a bare relative path across folders resolves against the session cwd and breaks the moment either skill runs from a plugin cache.
