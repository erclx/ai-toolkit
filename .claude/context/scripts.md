...
- An eval arm can write outside the fixture, and the before-and-after snapshot will not see it. A cut half of the memory ablation wrote into `~/.claude/projects/<fixture>/memory/` and the run reported no files changed, which is true of the fixture and false of the machine. Read the transcript alongside the file list, and clear the stray directory afterward.
