...
- Nothing prunes `.claude/.tmp/eval-runs/`. It is the second uncapped scratch folder after `sandbox-runs/`, and both wait on the scratch-lifecycle question `v19.0` exists to settle. Clearing it by hand loses every `Output` path the ledger points at, which is the intended trade rather than a bug.
...
| `verify.sh`                | `check`     | Repairs `core.bare`, then format, three drift stages, the skill-path, plugin-boundary, and context-citation guards, and spell always run. Shell, types, and tests gate on changed files unless `--all` |
