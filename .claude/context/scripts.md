...
- A run that costs money leaves two records, split by what each costs to keep. `scripts/eval/ledger.md` takes one appended row per run carrying date, arm, kind, subject commit, cost, turns, verdict, and output path, and is committed. The raw transcript lands in `.claude/.tmp/eval-runs/<arm>-<timestamp>/` and is not, because a transcript holds the full text of every file the session read and a hundred retained runs is a repository hundreds of megabytes larger for every clone. The row is the durable record and a transcript is promoted into the arm's result document by hand on the day it becomes evidence for a claim. Retention is additive, so a failure to write either one warns and the verdict still prints.
