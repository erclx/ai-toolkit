...
- The step reads `"removed":<digits>` on the success path and falls back to `"reason"` then `"failed"` on the failure path. A record arriving with `"reason":null` matches no pattern, which is what lets one block separate a refusal from a removal that failed.
- An unreadable reading refuses every verdict rather than some of them, so the step reports one line. `gh` absent or unauthenticated is the ordinary shape of that.
- Nothing automated tests the step itself. The sandbox drives model sessions against skills and fires no husky hook, so what stands in its place is the `--json` record under test at `src/commands/worktrees.test.ts`, a `shellcheck --shell=sh` pass, and a hand probe.
