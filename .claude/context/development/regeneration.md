...
`.claude/rules/` is not a whole-directory mirror, because the toolkit authors 72 rules under `governance/rules/` and consumes 64 into `.claude/rules/`, and installing the framework rules here would fire a React rule on a fixture this repository writes. The ui rules are consumed and the framework rules are not, which separates two families this sentence read as one until this repository started rendering surfaces of its own. `internal/governance.toml` carries that reason beside the list. It resolves through `canon gov regen` instead, which reads the stack named in `internal/governance.toml` and installs it with the same machinery `canon gov install` uses for a target.
