...
Three of the four are whole-directory mirrors. `.claude/rules/` is not, because the toolkit authors 52 rules under `governance/rules/` and consumes 40 into `.claude/rules/`, and installing the framework and ui rules here would fire a React rule on a fixture this repository writes. It resolves through `aitk gov regen` instead, which reads the stack named in `internal/governance.toml` and installs it with the same machinery `aitk gov install` uses for a target.
