...
A fourth trigger counts a repeated tool name, and the count runs after shell parsing rather than over the raw text. A command naming `git` more than once is refused, so `git branch -m worktree-<name> feat/<name>` fails whenever `<name>` carries that token, which is `claude-worktree` Step 5 for any branch under a git topic. Splitting the token across two adjacent quoted strings does not evade it, since the parser reassembles them before the count, while a command substitution spelling the bytes does. The same token inside an ordinary argument passes, so the count rather than the position decides. Measured 2026-08-28 on `feat/git-skill-precedence`.
