...
A plan that ships moves to `.canon/plans/archive/` under its original name, swept there by `claude-docs`. Deletion was the earlier policy and cost a shipped plan outright, because `.canon/plans/` is gitignored and nothing backs it up. A re-shipped slug overwrites the earlier file, which keeps the folder holding intact plans under the names they were written with.
...
A memory-review receipt a triage takes out of `.canon/review/memory/` moves to `archive/` beside it. It sat in the scratch tree first and failed the deletable test on content: the pass that filled it carried 154 receipts holding 261 undecided items, which `canon standards memory` calls decision state the next round reads back, and nothing there reaches the records remote. A receipt whose items have all resolved needs no archive at all, since the collection rule folds its declines into the entries and deletes the file.
...
The plan move takes the same skew in the quiet direction. A binary predating it archives the task and leaves the plan live, reporting success either way, so between the merge of that change and the release carrying it no plan archives at all. A plan left live is recoverable and one archived early is not, which is why the lag was taken rather than worked around.

The task archive verb itself takes the opposite path on the same skew. A stale binary still finds the board, still writes the file, and lands it in a flat `task-archive/` sibling rather than the `tasks/archive/` folder nested inside it, with neither the board count nor the exit code reporting anything wrong.
...
A rewritten file can still couple to an excluded one, naming an excluded prefix or path as plain text while carrying a citation of its own, which `referencesExcluded` reports on `RecordsPlan.coupled` as a third list beside `entries` and `excluded`. [#1429](https://github.com/erclx/canon/issues/1429) reported the shape this closes: a shell test's dual-root check surviving with only one root after the citation beside it was rewritten. The check reads the whole file's text rather than the citation's own line, since that is where the reported pair sat.

The two roots take different rules and the asymmetry is deliberate. `.canon/` is read whole, since `.claude/ARCHITECTURE.md` fixes the rule that every gitignored record moves there and nothing tracked ever lands there, which covers a folder `RECORD_ENTRIES` has yet to learn about. `.claude/` is mixed and has to be entry-scoped, because a bare prefix there drops 163 tracked files here and strands a target's installed `.claude/rules/canon/core/035-tasks.md`, which is the file the sweep exists to repoint.
...
Archiving moves a record between folders that all sit on one disk, so `canon records push` carries the nine of them to a private remote and `canon records pull` brings them back. They are the record root's own entries less three: `tmp`, deletable by definition, `.claude/worktrees/`, whose contents the project repository holds already, and `.records.git/`, which is the history the rest push into. Nothing bounds the nine any more, since the claude manifest used to ship a folder apiece and now ships one `.canon/` root entry that names none, so `BACKED_FOLDERS` being spelled out is the whole of the protection rather than half of it. `docs/agents/records.md` carries the verb surface and the refusal table.
...
Each pathspec is a bare folder name and git derives its prefix from the current directory rather than from the work tree the same call passes, so the invocation carries `-C` at the work tree beside `--git-dir` and `--work-tree`. Without it a session under `.claude/worktrees/` stands inside the records work tree, every name picks up that worktree's own path, and a push fails with `fatal: pathspec '<first folder>' did not match any files` naming whichever folder sorts first. `--root` never reached that, since the root is threaded correctly and the prefix does not read it, which is why the flag a caller reaches for first closed nothing. Both verbs run from anywhere now, and that is the shape the rule needs, since implementation work runs in a linked worktree. The two directions failed differently and pull was the dangerous one: `add` refuses an unmatched pathspec outright, while `status` tolerates one, so the gate that refuses rather than discarding an unpushed record read clean from inside the tree and the reset behind it would have taken the file.
