...
A binary predating the convention does more than write the old path. `resolveLivePlan` without an archive subtraction reads a `Plan:` line pointing into `.canon/plans/archive/` as live and refuses with `plan-unswept`, so the installed `canon tasks archive` declines every task whose plan has already shipped until a release lands. It fails closed rather than corrupting the board, which is the safe direction and still stops the archiving.
