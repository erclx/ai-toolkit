...
`runCli` asserts its own containment on every call rather than trusting a case to. `gov install` and `gov sync` both record the target they ran against into `stateDir()/targets.json` at `src/targets/registry.ts`, and a case inheriting the real `HOME` unmodified writes a row for a directory `afterEach` is about to delete, leaving a dead entry on the machine that ran the suite. `runCli` points `AITK_STATE_DIR` at a folder under the case's own `cwd`, the override `stateDir()` already reads first, and snapshots the real registry before and after every spawn regardless, throwing `ContainmentViolation` the moment the two disagree. The snapshot comparison is a pure function, `detectRegistryLeak`, so `harness.test.ts` covers the detection logic without touching the filesystem, and a `describe('containment')` case asserts the real registry survives a `gov install` plus `gov sync` run unchanged. `spawnSync` also carries a 10-second timeout, since a synchronous spawn cannot be interrupted by a framework timeout and a blocked verb would otherwise hang the whole run rather than failing one case.

The tier folds into `bun --bun vitest run` rather than a step of its own for this pass, so `bun run check` and CI both already run it. Fifteen cases across five verbs plus the harness's own containment coverage cost roughly 1.5 wall seconds and 4 CPU seconds, measured 2026-08-31, which is the number to recheck once the case count reaches fifty before assuming the tier stays cheap.
...
A line citation into `scripts/core/verify.sh` is a third copy nothing compares either. Declaring the array near the top of that file moved every guard down 17 lines, which invalidated the three citations above and a fourth in `.claude/context/development/gates.md` that had already drifted 29 lines before this branch touched anything. All four were repaired here, the fourth on the ground that a change widening a drift owns it whether or not it opened it. Re-read every citation into a file after moving anything above the line it names, since no stage compares a cited number against what sits there.
