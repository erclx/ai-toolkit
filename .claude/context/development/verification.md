...
### An orphaned `canon serve` process squats a fixed port across repositories

`src/serve/static.test.ts`'s "should walk past a port already in use" test binds `DEFAULT_PORT` (8787) directly, so a `canon serve` process left running from an unrelated session, in an unrelated repository, and holding that exact port fails the test with `bind-failed` on the first `start()` call, unrelated to whatever diff is under test. One such process was found still bound 46 hours after launch, parented to `/init` with no live session owning it, and a shared machine running several concurrent worktrees can carry more than one of these at once across different repositories. Read what the failure actually reports, `bind-failed` on the first bind rather than an off-by-one on the walk, and confirm which process holds `DEFAULT_PORT` itself, with `ss` or `lsof` over listening sockets, before diagnosing further, since a nearby port held by a different, unrelated process is easy to mistake for the cause. The fix is killing the leaked process, an operator action, not touching the test or the code under review.
