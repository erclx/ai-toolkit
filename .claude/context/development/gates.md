...
The marker mutes a line and nothing narrower, because `isMarked` reads the line itself and the one above and stops there. Nineteen lines carry it as of the docs-path and phase-label patterns landing, up from the six the pull-request and commit-sha patterns alone required: the original six illustrate the `#123` spelling in `standards/publish.md`, the `verified` field format in `standards/diagrams.md` and in `claude-diagram`, a copyable `canon claude skills drift` invocation in `docs/agents/skills-audit.md` whose argument has to be a literal git ref, and two same-repository citations in `docs/agents/context-audit-checks.md` and `docs/agents/key-changes.md`. The other thirteen illustrate a phase label's own format, spread across `docs/agents/tasks.md` (seven), `standards/tasks.md` (four), `standards/versioning.md`, and `claude/skills/claude-tasks/SKILL.md`. `docs/agents/comments.md`'s `--since v0.5.0` example needed one too until the negative lookahead excluded its three-group shape outright, which is the one instance this reader closes by construction rather than by marker. A real citation later added beside any of the nineteen ships unreported.
