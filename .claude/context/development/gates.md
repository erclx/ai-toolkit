...
The marker mutes a line and nothing narrower, because `isMarked` reads the line itself and the one above and stops there. Twenty lines carry it as of the docs-path and phase-label patterns landing, up from the six the pull-request and commit-sha patterns alone required: the original six illustrate the `#123` spelling in `standards/publish.md`, the `verified` field format in `standards/diagrams.md` and in `claude-diagram`, a copyable `canon claude skills drift` invocation in `docs/agents/skills-audit.md` whose argument has to be a literal git ref, and two same-repository citations in `docs/agents/context-audit-checks.md` and `docs/agents/key-changes.md`. The other fourteen illustrate a phase label's own format, spread across `docs/agents/tasks.md` (seven), `standards/tasks.md` (four), `standards/versioning.md`, `docs/agents/comments.md`, and `claude/skills/claude-tasks/SKILL.md`, one of them a semver tag rather than a phase label since the pattern cannot tell the two shapes apart either. A real citation later added beside any of the twenty ships unreported.
