...
- The seed-standards stage discovers its roots rather than listing them, walking `tooling/*/seeds/` and keeping the ones carrying a `.claude/`, so a new stack is covered without a script edit. It is also the one stage that suppresses a passing run's output, because the audit prints its own frame and `run_check` would nest it inside this script's on every push.
