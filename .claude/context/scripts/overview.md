...
- Headless picker behavior moved to `internal/rules/core/097-non-interactive.md`, which globs `scripts/**/*.sh` and `src/**/*.ts` so it loads when a picker is added rather than when someone thinks to check. `aitk gov install` and `aitk snippets install` are the live refusals, returning 1 with the valid names when the argument is missing, because defaulting there picked a whole stack or category for the caller.
