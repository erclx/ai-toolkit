...
- The two headless harnesses stay separate. `scripts/eval/run.sh` extracts its fixture to a `mktemp -d` outside the repository so no ancestor instruction file leaks in, which is the opposite of the sandbox's need to look like a real installed project. Merging them would cost one of the two its defining property. The path was `scripts/standards/authoring-test/` until `#668` moved it.
...
- Three arms stage a standards citation check they cannot yet assert. `claude/review`, `claude/ui-test`, and `claude/memory-review` each declare no `SANDBOX_INJECT_STANDARDS`, so the project copy of `standards/skill.md` is absent and the plugin fallback is the only route to the slug transform. Each stages a branch carrying a `/`, and the transform replacing it with `-` appears in no skill body, so the output filename would be evidence the citation resolved rather than evidence about the skill. The fifth standing limit puts that filename outside the snapshot, so each arm carries the claim as an `Unwired:` manual entry and asserts only that `.claude/standards/skill.md` is absent. That one assertion is what makes a later edit adding the flag go red at the point it lands.
- A demoted assertion is never inverted into `absent`. The filenames a wrong run would have chosen are left undeclared on all three arms, because no output reaches `.sandbox/` under any name and an `absent` entry would pass for the file being elsewhere rather than for the run being correct. That is the vacuous pass `manual` is excluded from the count to prevent, arriving through the back door.
...
- A determined destination for any output a skill writes to shared session scratch. `run.sh` runs `claude -p` with cwd `.sandbox`, which sits inside the toolkit worktree, so the toolkit's own `CLAUDE.md` loads through the ancestor chain alongside the sandbox's seeded copy. Both carry the rule sending `.claude/plans/`, `.claude/review/`, `.claude/memory/`, and `.claude/tasks/` to the main worktree root, and nothing decides which root wins. Measured 2026-08-02 across two runs that disagreed: `claude:review` wrote `review-feat-user-batch.md` into the toolkit's own `.claude/review/`, while `claude:ui-test` wrote `ui-checklist-feat-task-filter.md` to the correct path inside `.sandbox/`. Both filenames were right, so the slug is not what varies. When a run does escape, `snapshot_tree` covers `.sandbox/` alone, so it reports `writes: []` and `write_scope` sees nothing, which is the harness's stated failure mode of asserting nothing while reporting green.
...
The fifth is a defect and is tracked as its own task. It makes every slug-based assertion flaky rather than impossible, which is worse to declare: the arm goes red on correct runs and nothing in the verdict says why. Each skill deriving a slug per `standards/skill.md` writes into exactly the scratch the rule redirects, and no armed arm had declared such a path before `claude/review`, `claude/ui-test`, and `claude/memory-review` tried, which is why it stayed hidden. `scripts/eval/run.sh` already solves the same problem by extracting outside the repository, and its own comment names the ancestor chain as the reason.
...
Neither number weighs an arm by what it asserts. Three of the twelve, added on 2026-08-02, run one assertion each, that `.claude/standards/skill.md` is absent, which is a claim about the arm's own provisioning and a tripwire for a later edit rather than a claim about the skill under test. Everything those arms want to say about their skill is demoted, and their declared write scopes are silent for the reason the fifth standing limit gives. So the count reads as scenarios reached rather than behavior covered, and a reader taking 10 percent as skill coverage is reading past what the declarations say plainly.
