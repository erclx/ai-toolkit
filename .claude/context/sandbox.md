...
- `.gitignore` keeps its `.sandbox/` entry after the move. Nothing provisions there now, so the entry costs a line and covers a stray tree from an older checkout or a hand-set override. `assert_sandbox_dir_safe` is the live guard: it refuses a relative path, a home directory, a filesystem root, and anything under the main worktree root, the last because a path back inside restores the ancestor chain and nothing else would say so.
