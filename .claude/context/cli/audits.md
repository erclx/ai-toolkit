...
- Both ban sets and all five checkpoints are parsed out of the standards at read time rather than held in code. A hardcoded list means an author adding a banned word gets no enforcement until someone edits TypeScript, and the toolkit's stated principle is that the standard is the source. The trade is a scraper against prose, bounded two ways:
  - `bans.test.ts` asserts the parsed sets against the shipped standards, so a reformat fails loudly
  - a checkpoint falls back per number, with the legend naming which one did
- Three roots answer a standard, at `.claude/standards/`, the authoring root, then the corpus inside the package the CLI runs out of. `src/standards/read.ts` owns the order and `src/markdown/bans.ts` reads it, so the audit and `aitk standards <name>` cannot resolve differently.
- A project copy wins wherever one exists, since an installed standard is a seed a project edits and a package copy overriding it would discard that edit with nothing said. The fallback answers absence rather than staleness.
- The package copy fills `source` with `<aitk>/standards/<name>`, where every other root fills it with a repo-relative path. A published package can lag the session reading it, so naming the copy read is what keeps that skew diagnosable.
...
- The seed copy of the same hook keeps its awk parse, which is the seed's corpus deciding rather than a consolidation left half done. A scaffolded project may carry no `aitk`, where awk enforces something and the verb would enforce nothing, and `scripts/core/check-seed-independence.sh` catches seed content depending on the toolkit CLI. Revisit when a seed ships with a CLI dependency of its own.
...
- `FOLDERS_BY_KIND` therefore carries a list per kind rather than one path, and a `no-folder` message names every candidate. The four scratch kinds keep a single-entry list, so their message is unchanged.
