...
- Nothing fires it automatically, and that is the constraint the verb exists to answer rather than a gap in it. `.claude/hooks/standards-audit.sh` exits early on the scratch paths and `claude-standards-audit` reads changed files from git, which never lists a gitignored one.
