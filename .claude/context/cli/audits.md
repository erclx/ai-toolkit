...
- Required sections are the first measure that cannot decide from one entry, so the folder judgment sits in `missingSections` beside `measureFolders` rather than widening `measureEntry`. Any sibling answers a required section, because four domains here split across a folder and carry both in an `overview.md`, and a per-file rule would report every other child of all four.
