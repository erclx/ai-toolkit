...
- `PROJECT_ROOT` derives from `import.meta.url` rather than Bun's `import.meta.dir`, which the test runner leaves undefined and which made `src/exec.ts` throw on import there. Both spellings resolve the same directory under Bun, so the derivation is the whole of the change and it is what puts a module reading the package root within reach of a test.
