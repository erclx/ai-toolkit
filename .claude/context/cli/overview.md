...
- Every git call resolving a repository strips the environment through `gitEnv()`, since a hook exports `GIT_DIR` into each process it runs and it beats `-C`. The Bun spelling is `.env(gitEnv())` and the `execa` one is `{ env: gitEnv(), extendEnv: false }`. `git --version` is the one call that needs neither, resolving no repository at all. The `gh` invocation in `src/sync/git.ts` still runs unstripped and shells out to git to find its repository.
