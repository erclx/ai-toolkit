...
The CLI ships to the registry as `@erclx/aitk` with nothing built, so what the package carries is decided entirely by the `files` list. Three trees are excluded, and the one command backed by an excluded engine is what sets the pattern any later exclusion follows.
...
The two capture sources under `assets/` are authored differently and only one is asserted. `install.html` is hand-written from a real run. `hero.html` is generated by `scripts/core/regen-hero.sh`, which fills `hero.html.tmpl` from five catalogs so no count on the image is maintained by hand, and the `Hero` stage in `verify.sh` regenerates it and fails on the difference.
...
`bun install --global @erclx/aitk` is the install path a plugin user follows. Nothing builds. `bin` points at `src/cli.ts`, whose `#!/usr/bin/env bun` shebang runs the source directly, which is why `src/` ships as TypeScript with `tsconfig.json` beside it so the `@/*` alias resolves.
