...
- `aitk claude skills list` reads `claude/skills/` alone. Internal skills under `.claude/skills/` are excluded, since a count spanning both reports work that never installs into a target. Each entry carries a `requirement` boolean for the sibling `REQUIREMENT.md`. It reports presence and never a reason, and since coverage is universal a `false` is a gap to close rather than a recorded exemption. Nothing gates the rule, so the boolean is the only reading of coverage there is.
