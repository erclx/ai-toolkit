...
- A domain earns a command when the operation is mechanical, not when the domain is important. Tasks gained `archive`, then `pull-request` and `outcome`, all three of which take a decision a skill already made and apply it to a file. Creating a task proposes a phase label from the board and writes prose from a conversation, which is judgment, so it stays in `claude-tasks` and no `create` verb exists to tempt a caller.
- A structured edit to shared scratch is the second thing that earns a verb, and worktree isolation is why. `Edit` and `Write` refuse a main-root path from a linked worktree, a shell stream editor is banned for an in-place edit, and a verb resolving the root in-process is what is left. `pull-request` and `outcome` are the two the board takes. Creating a whole file earns nothing, since a heredoc through `Bash` writes it safely, which is why no verb exists for a plan or a review report.
- `aitk claude skills list` reads `claude/skills/` alone. Internal skills under `.claude/skills/` are excluded, since a count spanning both reports work that never installs into a target. Each entry carries a `requirement` boolean for the sibling `REQUIREMENT.md`. It reports presence and never a reason, and since coverage is universal a `false` is a gap to close rather than a recorded exemption. `aitk claude skills audit` sits beside it and gates that rule across both corpora, so the boolean reads the shipped catalog while the audit answers whether a working tree conforms.
...
- A refusal reason splits on whether the caller can act on it, once any caller is told to swallow a subset. `git-pr` skips silently on `no-board`, `no-match`, and `ambiguous`, because each describes a board where a guessed write lands on the wrong task. A malformed argument describes the command line instead, so it refuses as `bad-input` and falls to that skill's report branch. Sharing a reason across the two classes puts a caller defect inside a swallowed set, which is the silent loss the verbs exist to prevent.
- All three task verbs carry that split, not only the two a swallowing caller drives. `archive` answered both selectors with `ambiguous`, which routes `claude-tasks` to repair a task citation that is fine, so one mistake read as three different board defects depending on which verb saw it. A taxonomy that holds for one verb in a domain and not its siblings is read as a distinction rather than as the omission it is.
