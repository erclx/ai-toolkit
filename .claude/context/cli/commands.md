...
- `aitk claude skills list` reads `claude/skills/` alone. Internal skills under `.claude/skills/` are excluded, since a count spanning both reports work that never installs into a target. Each entry carries a `requirement` boolean for the sibling `REQUIREMENT.md`.
- It reports presence and never a reason, and since coverage is universal a `false` is a gap to close rather than a recorded exemption. `aitk claude skills audit` sits beside it and gates that rule across both corpora, so the boolean reads the shipped catalog while the audit answers whether a working tree conforms.
- `aitk claude skills drift <ref>` is the third verb under `skills` and the only one reading history rather than a tree, naming which shipped bodies were rewritten between that ref and `HEAD`. A session holds a skill body from the moment it loads one and re-invoking the skill replays the held copy rather than the file, so nothing else compares the two. The ref is required because `HEAD` is the only default available and would answer every run with nothing moved.
...
- A verb editing shared session scratch under `.claude/` resolves the root through `mainWorktreeRoot` rather than the working directory. The task record verbs and `aitk intake answer` share the reason: every worker runs in a linked worktree where the file-editing tools refuse a main-root path and the stream editors this repository bans are what a shell route reaches for, so the write has to resolve the root in-process.
- A verb landing several edits in one file takes the whole batch in one invocation rather than one call per edit. `aitk intake answer` scopes a call to a single cluster for that reason, since concurrent calls read the same file and the last write keeps only its own change. A repeatable flag carrying `<key>=<value>` is the shape, splitting on the first separator so a value carrying its own survives.
