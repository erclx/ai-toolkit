...
- Which mechanism decided is now visible in the report. `misplacedOrphan` gives the name-inferred orphan a notice naming the path under `projectSubdir` it belongs at, and the location-orphaned file keeps the generic line. The notice queues no change, since moving a file rewrites a path the project's own content may cite, so the sync reports the placement and leaves the decision with whoever owns the citations.
