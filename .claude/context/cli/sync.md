...
- `aitk sync --check` skips every orphaned entry in `src/commands/sync.ts`, so the destination reaches `aitk gov sync` and `aitk snippets sync` alone. Widening it changes what the `up to date (N local)` line means, since a local file is counted there rather than reported, so it is a decision of its own rather than a consequence of adding the notice.
- The name reserves `project` across the toolkit's own `governance/rules/` and `snippets/` source trees too, not only across a target's installed copy, and nothing else checks that. `src/gov/adapter.test.ts` and `src/snippets/adapter.test.ts` each carry a guard test asserting neither tree holds a `project/` category, which is what would fail first if a later toolkit category collided with the reserved name.
...
- `.claude/aitk/` takes a loose config file the toolkit owns and nothing else. Installed content a project reads and edits keeps its own folder, which is `.claude/rules/` and `.claude/snippets/`, and generated data keeps the folder its producer names, which is `.claude/audits/baseline.json`. All three are toolkit-owned and none of them moves, so ownership is not the test and being a bare file in the shared root is.
...
- The chain's shape follows what its reader needs, not one fixed contract. Tooling stores the full ordered ancestor chain, nearest first, because a stack extends another and a `--skip` run installs fewer layers than the leaf's chain reproduces, so the report loads the recorded stacks directly rather than re-resolving. Governance stores only the leaf `aitk gov install` was given, since `resolveRules` walks that stack's own `extends` ancestors internally and a reader asking it again needs nothing more.
...
A recorded chain is read first, through `resolveMissingRules` in `src/gov/stacks.ts`. It compares the target's current tree against `resolveRules(root, chain[0])` and needs no anchor at all, which is what makes it immune to the gap the fallback below carries: a rule that joined the stack before the target's governance anchor is not a permanent blind spot, since the anchor never enters the read. `collectMissing` on the gov adapter runs the same function, so `aitk gov sync` reports a `missing` entry per file for exactly what this section names by list.
