...
- A standard states its rule and never what enforces it here. These files install into projects whose hooks, scratch paths, and skill catalogs are their own, so a rule justified by this repository's audit hook or illustrated with this catalog's output filenames is wrong in a target and nothing reports it. Name the general condition and let the consuming surface name its own case. `prose.md` has taken a finding for this twice, first on its yield clause naming a path and then on the pre-publish scan naming the audit hook. The rule now lives in `standards/standard.md` under `## Scoping rules`, routed to every standards edit by `591-standard-authoring`, because a decision recorded here governs nothing an author reads while editing. `v38.0` recorded the corpus as corrected, and the sweep that produced that verdict covered only the two files its plan named. `diagrams.md` and `wireframes.md` were carrying three more, which is what the `v38.3` pass found and fixed.
- Every standard declares its own jurisdiction in a `## Scope` section, above the shape rules. A standard that specifies shape exhaustively and jurisdiction nowhere cannot refuse a rule, so a rule with no obvious owner lands in whichever standard sits nearest and the corpus grows by annexation. `prose.md` is the worked case: it began as voice and hygiene, took a banned-character scan correctly in `#694`, then took a phase-label check sourced from `versioning.md` in `#727`. Neither change was wrong and neither had a scope statement to fail against. A sibling `REQUIREMENT.md` per standard was the alternative and it restates what a standard already is, since a standard is a specification and a skill body is not.
...
- A jurisdiction exclusion is not a content exclusion. `## Scope` names a concern another standard owns. `## What does not go in` names what does not belong inside the document. Eight standards carried the second under two spellings and none carried the first, so the sweep moved up only the bullets that named a sibling and left the rest where they were. Merging the two puts a boundary claim where no sibling will read it.
- A scope statement stays silent on a section the standard holds but should not own. Claiming it makes the statement false the moment the section moves, and the visible mismatch is the evidence that moves it. `prose.md` and `bundled/pr.md` each ship with one, the pre-publish scan and the testing-discipline rules, both queued for relocation under `v38.4`.
