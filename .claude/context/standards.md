...
- Both govern a gitignored folder no check reaches, which puts them in one class with the plan and memory standards rather than leaving each to find its own answer. `.claude/hooks/standards-audit.sh` exits early on the scratch paths and the audit skill reads changed files from git, which never lists a gitignored one, so all four are enforced by a session reading them and by nothing else. The precedent to extend is `aitk tasks validate`, which reports against the board without writing, and reporting is what makes a verb safe over a folder with no history to recover from. Four surfaces and one verb shape is the outcome to aim at, so the task closing the gap for plans and memory sizes its scope against four rather than two.
