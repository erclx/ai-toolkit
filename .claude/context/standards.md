...
├── aitk/                ← toolkit-internal standards, consumed in place, never installed
...
## Toolkit-internal standards

A convention that governs a surface only the toolkit repo has lives in `standards/aitk/` rather than the top-level `standards/`. `tooling-reference.md` is the case: target projects receive a rendered `.claude/tooling/<stack>.md` from `aitk tooling ref` and never author one, so installing its authoring rules into every project is noise. Install and list both walk the source with `-maxdepth 1`, so the subfolder drops out with no filtering code, unlike the `aitk` snippet category which needs an explicit exclusion list. `regen-claude-copies.sh` still mirrors it, so the toolkit reads it at `.claude/standards/aitk/<name>.md` and internal skills cite the same path shape as any other standard.
