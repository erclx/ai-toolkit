...
- The standard had to fix one spelling rather than record the existing one. `claude-feature` templated `**Constraints:**`, `**Files to touch:**`, `**Risks:**`, and `**Questions:**` as bold labels while every plan in the tree wrote `## Risks` and `## Questions` as headings, so the format was undecided in practice and a check over it would have had to accept both. H2 headings won, because `markdown.md` mandates H2 for a main section and three of the five markers were already headings. Small mode keeps bold labels, since it writes to the conversation rather than to the file the standard governs.
