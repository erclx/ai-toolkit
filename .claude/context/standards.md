...
- A procedure several skills execute mid-run gets a standard of its own in the flat root rather than a section inside whichever standard already held one of its rules. `standards/publish.md` and `standards/slug.md` are the two, each cited from every body that runs it. Both take a one-word name, which is what every other installable standard carries, and `slug.md` also drops the adjacency `branch-slug.md` had to `bundled/branch.md`. They shipped as sections of `prose.md` and `skill.md`, which owned one rule each procedure applies and neither procedure itself, and a scope statement that stayed silent on both is what made the mismatch arguable from the file. A subfolder was not available, since install and the sync adapter walk the flat root only. A combined `procedures.md` was the other candidate and it recreates the aggregation at a new address.
...
- A scope statement stays silent on a section the standard holds but should not own. Claiming it makes the statement false the moment the section moves, and the visible mismatch is the evidence that moves it. `prose.md` held the pre-publish scan on these terms until `v38.4` gave it a file, which is the route worked once. `bundled/pr.md` still holds its testing-discipline rules and takes the same route next.
