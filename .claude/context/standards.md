...
- A template and a seed can hold the same skeleton and drift apart. `tooling/claude/seeds/.claude/DESIGN.md` is the live case, and the template was written to match it column for column. Nothing compares the two yet, which the queued seed-conformance work is what would close.
