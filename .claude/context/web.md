...
- `readCatalogCounts` in `counts.ts` calls `spawnSync` for `canon gov counts --json`, never `execSync`. `execSync` throws on any non-zero exit, but the verb can exit 2 for drift elsewhere in the tree that this page's build did not cause. `spawnSync` lets the reader check `stdout` and fail only when it is genuinely empty, so the page never ships a count nothing measured.
