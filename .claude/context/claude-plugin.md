...
The empty list is what stops the chain instead. Routing it into review rather than stopping would re-create the silent skip by a longer path, since a review of no files produces no findings and Step 6 reads that as a clean pass. The prose-only skip survives untouched, because the defect was a broken read making every branch look prose-only rather than the skip itself. A plan whose output is entirely gitignored still reaches the stop rather than a fix, which is a separate defect that surfaces six steps later at `git-stage`.
