...
A `git init` project on `main` with no remote resolves no usable baseline, which is the ordinary shape of a scaffolded target project rather than an edge case. There the two diff commands are both empty, so the marking step reads `git log -p -1` and the working tree instead of a diff. The wireframe sweep and the context refresh skip with a warning, because both write, and a step that treats the whole tree as changed would stub a wireframe per uncovered surface and rewrite every context entry. Widening what a step reads is safe. Widening what a step writes is not.
