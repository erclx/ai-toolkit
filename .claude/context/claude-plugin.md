...
What the gate reads is a section's presence rather than a shipped flag. `standards/requirements.md` gained a Lifecycle section stating that later scope arrives as a new section and that a roadmap sequences the MVP list alone, so a file carrying a section outside the fixed six is one whose MVP list has already shipped. Nothing was added to mark a feature shipped, because the same section forbids annotating MVP entries with status and a flag would have been the thing it bans. `## Distribution` falls on the gated side, since the standard adds it conditionally rather than fixing it, and a project that wrote that section before shipping its MVP is stopped wrongly and recovers through the override the stop names. The remaining gap fails safe: a shipped MVP with no later section reads as unshipped and drafts, which is correct, since a project with no later scope has nothing a fresh pass would sequence.
