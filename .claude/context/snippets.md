...
- `internal/snippets/` owns toolkit-internal runbooks, outside anything the plugin ships
...
- Every folder under `snippets/` is publishable, so nothing filters. Internal runbooks live in `internal/snippets/`, which no entry point reads.
