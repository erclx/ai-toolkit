...
These rules live only in the target. `aitk gov sync` skips any rule with no toolkit source match, classified as `orphaned` by `planSync` in `src/sync/engine.ts`, so project-authored rules survive sync untouched. A rule under `.claude/rules/project/` is orphaned by location instead, since the gov adapter declares that subfolder on `SyncAdapter`, but `create-rule` keeps writing into the shared per-band subdirs rather than that folder, so the name inference is what actually protects its output today. Numbering bands are shared with toolkit rules, so a project rule and a toolkit rule must not share a number within the same subdir.
