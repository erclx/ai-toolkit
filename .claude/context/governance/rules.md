...
- `aitk gov counts` does not close the row above. It parses a self-stated count of skills, standards, snippets, commands, and audits, and on 2026-08-28 it named the two stale skill figures while reading past both rule figures, since neither entry phrases its count in a shape the matcher reads. The manual re-measure stays load-bearing and the audit clears a tree holding two wrong numbers.
