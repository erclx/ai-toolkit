...
- A rule authored under `governance/rules/` in a folder no stack names installs for nobody through `aitk gov install`. `.claude/context/governance/stacks.md` covers which folders close that and which do not. `governance/rules/snippets/` used to be the one folder that reached a target anyway, through `aitk snippets install`. That install channel is retired, so the folder now reaches only a target that already held it from before, the same stranded state a rule under any other unclaimed folder would sit in. See `.claude/context/snippets.md`.
