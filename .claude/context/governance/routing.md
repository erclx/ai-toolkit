...
Seventeen of the nineteen claude rules now carry operative directives beside the pointer, since a rule arrives attached to the edit while a pointer reaches a session only if that session opens the file. Five bullets is the cap, matching the two largest of the seven that reached the shape on their own, and past it a rule reproduces the standard's structure rather than stating what must not go wrong. The standard keeps the full specification either way, so a drift is a rule falling behind rather than two files disagreeing, and nothing checks it.

`500-prose` and `501-markdown` stay pointer-only, on two different arguments now that the parser exemption behind the old shared one is empty. `501-markdown` points at a standard whose bans ship as package data, so restating even part of the list here puts a second copy beside the one `aitk markdown audit` reads. `500-prose` points at the `write-human` skill rather than at a standard, and the rule carries the instruction to load it rather than a compressed sample of what it says, since the compressible half of that guidance is the half that does the least work. `561-teach` took the same form on its glossary bullet once that reference moved into the fan-out.

The cross-split pointer is the shape every rule citing a standard carries. All eighteen name `standards/<name>.md` inside the aitk plugin rather than an installed path, and each states the report-the-gap response in place of a fallback path. What forces it is that a rule has no code between its text and the file it names, so the fallback branch a skill body spells is unavailable and no verb is reachable either, which leaves the one path resolving wherever the plugin does. The corpus ships with the plugin while the rule ships with the CLI, so the two arrive separately and nothing detects the gap at install time.
