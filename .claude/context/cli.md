...
- A branch gated behind a prompt takes an optional decision seam so a test can reach it. `select` exits without a TTY, which left the `aitk sync` staging and commit branches coverable only by driving a PTY by hand. `WorkflowDeps.choose` defaults to the real prompt, so the command layer omits it and production behavior is unchanged, while the tests assert the exact staged set. Prefer this over asserting on the logic that feeds a prompt and calling the branch behind it covered.
