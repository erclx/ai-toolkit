---
title: Inventory
description: The route walk behind aitk inventory, why it reports a listing rather than a verdict, and the focus reader's dependence on keyboard modality
---
...
`aitk inventory` walks every route a project declares and reports one computed property per element, grouped by the answer rather than by the component. The command surface and its refusal reasons live in `docs/agents/commands.md`. This entry holds why it is shaped the way it is.
