...
Three findings shape the rule. Any posted finding is the gate, which is the threshold `claude-pr-review` states and posts its open heading under, so the heading and the counts beneath it answer the dispatch question alike. A session resolves at the moment of sending and never from a stored map, because names rotate and one written down earlier in a session failed inside the hour. `aitk sessions list --branch` is what performs that resolution, and it reports a count and a confidence rather than a row, so the routing reads how many sessions came back and how liveness was decided rather than addressing the first one. Reading the target off a bare listing was the alternative and it lost on measurement. The listing returned five rows carrying a name, a ref, a kind, a state, and an age, four of them sharing one project prefix and two of those four holding different branches, so a resolution that got the project right still had a one in four chance of getting the branch right. A dispatched worker is named from its task rather than from its directory besides, so the prefix a reader would match on is absent rather than merely weak. A message carries plain text, so the step names a skill for the reader to run rather than embedding an invocation that would arrive as text.
