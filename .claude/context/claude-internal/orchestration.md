...
Counting `.claude/task-archive/` beside the board is what separates the two, and the answer slots are read through `aitk intake list --json` rather than by grepping headings, since the verb owns the parse `.claude/standards/intake.md` fixes and skips the index and every fenced sample a grep would count. Both reported states are empty as of 2026-08-20, measured at `e5abee70`, which is why the step names what it read rather than printing nothing on a clean pass.
