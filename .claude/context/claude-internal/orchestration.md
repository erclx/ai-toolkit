...
The archive gate on that ordering counts citations rather than reading a folder. `otherTasksCitingPlan` in `src/tasks/archive.ts` names the other live tasks whose `Plan:` line resolves onto the same file, and only a task that is the last one holding a live plan is refused. Testing the folder alone was what shipped first, and it deadlocked against the sweep: three tasks cited one workspace plan, the sweep was correct to leave a plan another live task cites, and the gate then refused all three with neither half in the wrong. Both halves resolve the target against the board and against the project root, so `../plans/x.md` and `.claude/plans/x.md` count as one citation. `aitk tasks plan-citations` exposes the same answer as `location` and `citedBy`, and reports without writing, since the move and the retarget are the sweep's to order.

The `claude-docs` body still states the rule instead of calling that verb, and the reason is the publish lag `.claude/ARCHITECTURE.md` already names as an open risk. A skill reaches a target on merge while the CLI reaches one on release, so a body calling a verb the installed `aitk` predates gets no record back and routes on nothing. Pointing the sweep at the verb was tried and measured on the `claude:docs` `board-sweep` arm: six of twelve assertions failed, neither plan was archived, and `.claude/plans/archive/` was never created. The duplication is accepted until a release ships the verb, and switching the body then, reading the record rather than the exit, is what retires it.

Nothing in the run reported the failure, and the reason for that is wider than this branch. An operator's shell profile may wrap `aitk` in a function that runs the binary and then a second command, and the one on the authoring machine ends in `cd .`, so the function returns 0 whatever the binary did. It masks an ordinary refusal exactly as it masks an absent verb, measured against `0.98.0`: `aitk tasks archive` against no such stem exits 1 called directly and 0 called through the wrapper. Every task verb already tells a caller to branch on `reason` rather than on the exit, which is the rule that survives it.
...
The reverse direction of that invariant reads three origins, and the dump is the one it reached last. `claude-tasks` listed tracks and ran an issue query while the word `intake` appeared nowhere in its body, so nine folders holding 199 answered items were covered by no route at all. What kept the fix from being a fourth grep is that the obvious signal reports the opposite of the truth. Six of the nine have no live task and every one of those six is cited by tasks in the archive, which is promoted and shipped rather than abandoned, so a check keyed on board citations alone names six finished folders and none of the state it exists to find.

Counting `.claude/tasks/archive/` beside the board is what separates the two, and the answer slots are read through `aitk intake list --json` rather than by grepping headings, since the verb owns the parse `.claude/standards/intake.md` fixes and skips the index and every fenced sample a grep would count. The verb reports four counts per folder and the check reads three of them. `malformed` earns its own wording rather than an exclusion, because an item carrying no answer slot is neither unread nor answered, so a test reading `unread` alone folds a broken file onto the answered side and reports it as decided work nobody promoted. All three reported states are empty as of 2026-08-20, measured at `e5abee70`, which is why the step names what it read rather than printing nothing on a clean pass.
...
`RESPONSE` is two tests rather than one count. The count asks whether a reply is new to the script, the stamp asks whether it is newer than the last pass, and a reply failing the second is one that pass already answered. A worker answers a finding and the reviewing session closes out seconds later, which is the ordinary handback rather than a race, so the count alone reported the answered thread on the next run and the re-review it routed to stopped at the guard `claude-pr-review` states.
...
`aitk sessions list --branch` performs that resolution. It answers with a count and a confidence rather than a row, so the routing reads how many sessions came back and how liveness was decided instead of addressing the first one.
...
The dispatcher names the branch on the launch and the worker takes it. Letting each derive its own put two independent judgments on one string and both halves have now disagreed in production, once on the slug and once on the type. The check then verified a branch nobody used, and the run died at its first step, since `claude-autoship` resolves its plan from the slug the worker actually took. Passing the plan path explicitly was the fuller fix and it needs an argument `claude-autoship` does not accept, where a dispatcher-named branch satisfies that guard by construction.
