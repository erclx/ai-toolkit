...
`.claude/plans/`, `.claude/review/`, `.claude/memory/`, and `.claude/tasks/` are gitignored and live at the main worktree root. A linked worktree reads them there rather than keeping its own copy.

Writing them from a linked worktree is refused. The session is isolated to its own worktree and a write to the shared checkout returns a message naming the isolation rather than a permission error, so a skill whose guard sends output to the main root has nowhere to put it and falls back to a worktree copy that is gitignored and reaches no later session. `claude-autoship`, `claude-review`, and `claude-memory-capture` each state the main-root rule in their guards and each meet this, which leaves the board, the plans, and the memory pen writable only from the main checkout.
