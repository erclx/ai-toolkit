...
A memory entry that leaves the folder moves to `.claude/.tmp/memory-archive/`, matching the plan and task archives. Deletion was never safe here: the folder is gitignored, so a wrong call in a bulk pass over a folder this size has nothing to recover from.
