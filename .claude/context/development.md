...
- `.cspell/banned-spellings.txt` is a third dictionary holding the British spellings `aitk markdown audit` reports. They sit apart from `project-terms.txt` because that file reads as vocabulary this repository writes, and these are the closed set a check exists to find. Folding them in would spell-approve the words the audit is built to flag, so a test or a doc naming one adds it here.
