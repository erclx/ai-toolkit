...
A linked worktree can read them and cannot write them. `Edit` and `Write` refuse every main-root path with a message naming session isolation and directing the session to the worktree copy, while `Read` resolves normally and `Bash` writes without complaint, so the boundary is tool-scoped rather than filesystem-scoped. Measured 2026-08-05 from a linked worktree against all four folders and a live task file. `git-pr` writes the `Pull request:` line through that refused path, so a pull request opened from a linked worktree cannot record its number and the archive gate below never gets one. Two tasks merged without the line and both carried a matching `Plan:` line against a live board, which excludes two of the three conditions under which `git-pr` skips that write silently and leaves the refusal as the explanation that stands.
