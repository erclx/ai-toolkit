...
That exemption is empty and the reason it emptied is recorded in `.claude/ARCHITECTURE.md`. The three ban sets ship as package data `canon markdown audit` reads, so no standard is parsed and the rule pointing at `markdown.md` may carry operative directives like any other.
