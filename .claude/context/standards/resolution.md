...
- A standard citing a `docs/agents/` page resolves for a reader in this checkout and for nobody else. Neither root carries a `docs/` tree, and no `aitk docs` read verb exists to name the page the way `aitk standards <name>` names a sibling standard. `standards/tasks.md` is the first standard to carry one, taken deliberately when the CLI-contract prose it restated moved to `docs/agents/tasks.md`, and the context-citation stage passes it because the path resolves here.
