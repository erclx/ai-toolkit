...
The `claude-docs` body still states the rule instead of calling that verb, and the reason is the publish lag `.claude/ARCHITECTURE.md` already names as an open risk. A skill reaches a target on merge while the CLI reaches one on release, and commander answers an unknown subcommand with `unknown command` on exit code 0, so a body calling a verb the installed `aitk` predates reads success, gets no record, and routes on nothing. Pointing the sweep at the verb was tried and measured on the `claude:docs` `board-sweep` arm: six of twelve assertions failed, neither plan was archived, and `.claude/plans-archive/` was never created, with no error anywhere in the run. The duplication is therefore accepted until a release ships the verb, and switching the body is the change that retires it.
