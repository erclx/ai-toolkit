...
Three findings shape the rule. Any posted finding is the gate, so the dispatch reads the counts on the summary line rather than the heading above them. A session resolves at the moment of sending and never from a stored map, because names rotate and one written down earlier in a session failed inside the hour. A message carries plain text, so the step names a skill for the reader to run rather than embedding an invocation that would arrive as text.
