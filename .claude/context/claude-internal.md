...
Three orchestrator snippets cover the moments the model cannot detect on its own. `snippets/claude/orchestrator-sweep.md` triggers the queue refill after a batch of merges. `snippets/claude/orchestrator-handoff.md` writes `.claude/tasks/session.md` before a compaction with what no other artifact carries, and `snippets/claude/orchestrator-resume.md` reads it back afterwards alongside the board and the groundwork behind the live work.
