...
`orchestrator-handoff.md` covers the write side of a compaction and `orchestrator-resume.md` the read side, both over the session map `.claude/standards/session-map.md` governs. The standard holds what any session can fill, which is the three core sections, the per-session filename, the capture and drift steps that open the write, and the citation rule. The runbook holds what the role adds over it: the section for decisions taken under delegated authority, the closing block that restarts the review poll, and the instruction telling capture this session never commits.
