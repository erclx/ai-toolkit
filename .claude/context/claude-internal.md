...
`orchestrator-handoff.md` runs memory capture, reads which skill bodies moved under the session through `aitk claude skills drift`, and then writes `.claude/tasks/session.md` before a compaction with what no other artifact carries, and `orchestrator-resume.md` reads it back afterwards alongside the board and the groundwork behind the live work. The drift read sits ahead of the write because a body the session has been following out of date would otherwise shape every line below it, and the verb answers in this repository alone, since a project consuming the plugin from a cache has no history behind the skills tree.
