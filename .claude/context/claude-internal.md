...
The archive gate on that ordering counts citations rather than reading a folder. `otherTasksCitingPlan` in `src/tasks/archive.ts` names the other live tasks whose `Plan:` line resolves onto the same file, and only a task that is the last one holding a live plan is refused. Testing the folder alone was what shipped first, and it deadlocked against the sweep on 2026-08-19: three tasks cited one workspace plan, the sweep was correct to leave a plan another live task cites, and the gate then refused all three with neither half in the wrong. Both halves resolve the target against the board and against the project root, so `../plans/x.md` and `.claude/plans/x.md` count as one citation. `aitk tasks plan-citations` exposes the same answer as `location` and `citedBy`, and reports without writing, since the move and the retarget are the sweep's to order.
...
Severity was the gate first, and the trial had already graded it poorly in both directions: a mis-called minor dispatched nobody and landed as a defect the operator had to notice, while a should-fix raised over a decision the plan already declined sent a worker to act on nothing. A live minors-only pass on 2026-08-14 sat until the operator ran the address skill by hand, which is what took the floor off. The second direction is unaddressed by that, since a dispatch on any finding still sends a worker to a should-fix the plan settled, and the return leg below is what answers it rather than a grade nobody calls consistently.
