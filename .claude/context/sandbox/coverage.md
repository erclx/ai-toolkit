...
A run on 2026-08-28 passed at 14 asserted and 0 failed with 6 unchecked, in 10 turns at 0.23 dollars. The split came back 1 unchanged, 1 drifted, and 2 matching no standard, which is the fixture. Four of the six manual entries were confirmed by reading the reply: the sync is named ahead of the drop, the project-local skill repoints to `aitk standards slug` rather than to the plugin-root form, `prose` lands under `You supply` rather than taking a one-to-one repoint, and `slug` and `branch` sort into the right buckets. The other two are unwired, needing an arm that stages a stale binary and one that leaves the tree out of the index.

The cap stays at 30 against that one observation. Ten turns is a floor on what a correct run costs rather than a bound on it, the same reading `superseded` above records for its own single sample.
