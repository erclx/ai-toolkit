...
What let them be asserted at all is `reply`. The six arms pin the skill or command each route names, which is the mechanism `05-testability.md` called for and the arms shipped beside the drift report did not use, so the report was verified against three arms and six real targets while the router reading it had never been executed. Splitting the route from the proposal is what keeps each arm scoring one decision: `claude:toolkit-operator/unmigrated` asserts that the reply reaches `migration-standards`, and `claude:migration-standards/root-layout` asserts what that skill then produces.

A reply pin asserts phrasing, so each one is paired with a `manual` entry naming the negative a substring cannot carry. Five of the six arms also assert over the tree, in two directions. Four pin what a run must leave alone, since the skill may execute nothing there: the staged root layout is still at the root, nothing appeared under `.claude/`, and the audit arm's staged entry and plan survive a pass that only measures. The `gitignore` arm pins what the write produced instead, the managed entries back in the file and the install stamp left as the inject wrote it, which separates the narrow mode from a full inject running under its name. The `fresh` arm carries no such pin, because a handoff to `setup-init` may legitimately continue into that skill and write the whole scaffold, and no path assertion separates the router doing the work from the router routing to something that does it.
