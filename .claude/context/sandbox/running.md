...
scripts/sandbox/run.sh claude:feature "/canon:plan-feature add a widget" small
```
...
A positive `reply` entry is only as stable as the slot it reads, so pin a token the fixture fixes and send a token the run chooses to `manual`. `claude:orchestrate` supplied the case: run twice in succession on 2026-08-20, against one unchanged fixture on one unchanged branch, it returned `pass` at the 6 asserted it then carried and 0 failed, then `fail` at 6 asserted and 1 failed. The entry that moved was `plan-feature`, which reads the skill the board report proposes on its `Next:` line, and that arm's own `manual` bucket already recorded the same line as beyond a string match. Read a single failure of that shape against a second run before calling it a regression.
