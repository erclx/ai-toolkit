...
A positive `reply` entry is only as stable as the slot it reads. `claude:orchestrate` run twice in succession on 2026-08-20, against one unchanged fixture on one unchanged branch, returned `pass` at 6 asserted and 0 failed, then `fail` at 6 asserted and 1 failed. The entry that moved was `claude-feature`, which reads the skill the board report proposes on its `Next:` line, and the `manual` bucket already records that line as beyond a string match. Read a single failure of that shape against a second run before calling it a regression, and prefer a token the run derives over one it chooses.
