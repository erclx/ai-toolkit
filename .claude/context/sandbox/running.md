...
The reply half still passes, because the reply comes off the envelope the run produced, while every path, absence, and content assertion reads the tree that replaced it, so the arm reports a mixed verdict that looks like a skill defect rather than a collision. One run was lost this way on 2026-08-04, reporting five failures against a tree holding another scenario's anchor fixture.
