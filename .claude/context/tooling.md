...
`canon tooling sync` carried the identical trap. A bare `canon` on PATH resolves `PROJECT_ROOT` to the installed package rather than the checkout a caller stands in, and `sync` carries no internal reroute the way `verify.sh`'s Sync phase reroutes its own call. `findCheckoutMismatch` in `src/project-root.ts` walks upward from `process.cwd()` for the nearest ancestor `package.json` sharing this package's own name, and `runSync` warns rather than refuses when that ancestor differs from the resolved `PROJECT_ROOT`, naming both roots and the fix command. `create`, `inject`, `list`, `reference`, and `prune-gitignore` read `PROJECT_ROOT` the same way and were not measured for the same trap.
