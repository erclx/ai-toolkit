...
- This repository's own `.gitignore` splits its Claude entries across two headers, with `.claude/teach/` under `# Teaching workspace` rather than under `# Claude`. Diffing the manifest group against the first header alone therefore reads one entry short, so `scripts/core/check-ignore-parity.sh` compares against every `.claude/` and `.canon/` line in the file, with the trailing slash dropped so `.claude/.tmp` and `.claude/.tmp/` read as one path. `.canon` joined that scope alongside the ignore entry itself, since a pattern outside the case the loop matches on is one the check passes having compared nothing, and the new root is the one the record folders are moving to. It gates `bun run check` rather than reporting, because the manifest reaches every target on the next `canon tooling sync` and a drift between the two lists surfaces to nobody. The gap it was written against was `.claude/proposals/`, ignored here and missing from the manifest, found by a person reading a merged pull request rather than by any check.
- Two divergences stay, `.claude/README.md` and `.claude/diagrams/`, each with its reason beside it in the check's own `SANCTIONED` list and printed on the run that passes. The diagrams answer follows the decision that kept `diagrams` in `DEFAULT_FOLDERS` in `src/context/folders.ts`: removing it there would take the audit default from every target that does track its diagrams, and ignoring the folder for every target reaches the same population from the other side. `BACKED_FOLDERS` in `src/records/backup.ts` names `diagrams` regardless, so a target's own `canon records push` carries the folder whether or not that target ignores it, an asymmetry recorded rather than resolved. A sanction naming a path that is no longer divergent fails the same as an unsanctioned one, so an exception cannot outlive its reason.
