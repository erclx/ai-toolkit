...
The command's own Sync phase cannot validate a branch's edit to `tooling/manifest.toml` or the injection engine, since it shells to bare `canon` in `scripts/tooling/verify.sh`, resolving whatever install sits on PATH rather than this checkout's source. Measured 2026-09-04: running it from a branch pinning `typescript` to `^6` still installed `^7.0.2` from an unpatched global install. Validate a branch's own tooling edit by hand instead, replaying the scaffold and any `[verify] prepare` command, then `bun src/cli.ts tooling sync <stack> . --write` from local source.

Two further gaps surfaced the same day, both upstream of this checkout. `astro preview` now backgrounds itself by default in the astro release the scaffold resolves, so Playwright's `webServer` config meets `Process from config.webServer exited early` before any browser launches, on both stacks' shipped `playwright.config.ts`.

`vite-react`'s raw scaffold ships its own `"lint": "oxlint"` script and a bare `.oxlintrc.json` that `applyScripts` never replaces, since it only fills a script key the target lacks, and the oxlint version that scaffold resolves today reports `No files found to lint` on the untouched scaffold alone. That fails `bun run check`'s `Lint` stage ahead of `Spelling`, but the validator's own `test:e2e` and `screenshot` phases still run regardless, since neither gates on the `check` phase's exit.
