...
- The overwrite contract names every config path rather than the category. `scripts/core/regen-tooling-paths.sh` writes the block in `claude/skills/aitk-cli/SKILL.md` from `aitk tooling list --json` crossed with each stack's `configs/`, and `bun run check` asserts it against the tree. Reading the list from the verb rather than from a directory walk keeps an excluded stack out of a contract about what that verb does.
