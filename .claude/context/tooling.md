...
Measured 2026-09-06 against the shipped `tooling/astro/configs/eslint.config.js`, in an isolated scratch install rather than a committed devDependency. A real ESLint run against a PascalCase `.astro` fixture (`MyComponent.astro`) reported nothing for `check-file/filename-naming-convention` under the shipped config, confirming the config-reading finding that the option's `.astro` pattern sits inside a block scoped to `**/*.{ts,tsx,js,jsx}` and never reaches it. The same run against a deliberately bad-cased `.ts` fixture did fire the rule, so the silence on `.astro` is a real gap rather than a broken harness. The first narrowing tried, `**/*.{ts,tsx,js,jsx}` to match the block's own selector exactly, turned out to newly enable the rule against `.js`/`.jsx` filenames, which neither `tooling/web/configs/eslint.config.js` (`**/!(App).{ts,tsx}`) nor `tooling/nextjs/configs/eslint.config.js` (`**/*.{ts,tsx}`) enforces despite selecting those extensions in the identical block, confirmed against `micromatch.isMatch` directly. The shipped pattern is `**/*.{ts,tsx}` instead, which matches both siblings, still stops naming a type the block cannot select, and leaves the `.astro` fixture unflagged either way: Astro's own PascalCase component convention governs there instead of a newly enforced kebab-case.

`check-file/folder-naming-convention` turned out to reach no folder at all, `.astro`-only or otherwise, under either version of the config. A PascalCase folder holding only an `.astro` file and a separate PascalCase folder holding a `.ts` file both linted clean. The rule's own implementation calls `micromatch.capture()` against its configured pattern to find the path segments to check, and the shipped pattern, `src/**/!(__tests__|pages)` with no trailing slash, returns `undefined` from that call for every directory tested, which the rule reads as nothing to check and skips. Adding a trailing slash (`src/**/!(__tests__|pages)/`) restores a real capture, confirmed directly against `micromatch` outside ESLint. The rule is inert for the whole stack today, not narrowly for `.astro`, and fixing the pattern is a separable change this measurement does not take.
