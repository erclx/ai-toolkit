...
A further gap surfaced the same day, upstream of this checkout and astro-only rather than shared across stacks. `astro preview` now backgrounds itself by default in the astro release the scaffold resolves, so Playwright's `webServer` config meets `Process from config.webServer exited early` before any browser launches. `tooling/astro/configs/playwright.config.ts`'s `webServer` block now sets `env: { ASTRO_PREVIEW_BACKGROUND: '0' }`, which keeps Playwright's own spawned process foregrounded without touching the shared `preview` script a human or an agent still runs by hand, verified by five consecutive clean runs where the unpatched block failed two of three. `vite-react`'s shipped config needed no equivalent fix: its `webServer` command runs plain `vite` through `bun run dev`, which stays foregrounded on its own and passed three consecutive runs against the untouched scaffold. Measured 2026-09-04.
