...
- `governance/rules/ui/440-surface-capture.md` now fires on every component file, reversing the route-and-page-only scope this entry once recorded. `#1505` shipped seven new component files with no capture attached, and a real defect went unseen inside them. Wide globs with the discrimination moved into the rule body, the alternative this entry once rejected, is the route the widened rule takes: a component the production build strips out, such as a dev-only scaffold switcher, carries its own exemption in the rule body rather than sitting outside a narrower glob. The four sibling rules in that folder still scope by file extension alone. This one keys on a path and states its exemption beside it. The accepted cost moves rather than closing: a component that ships to production and renders no surface of its own, a provider or a layout wrapper, now fires the rule too, since the exemption above reaches only what the build strips out. Measured at `64a4297b` on 2026-09-07.
...
`canon tooling sync` carried the identical trap. A bare `canon` on PATH resolves `PROJECT_ROOT` to the installed package rather than the checkout a caller stands in, and `sync` carries no internal reroute the way `verify.sh`'s Sync phase reroutes its own call. `findCheckoutMismatch` in `src/project-root.ts` walks upward from `process.cwd()` for the nearest ancestor `package.json` sharing this package's own name, and `checkoutMismatchWarning` beside it formats the line naming both roots. Every verb that answers from `PROJECT_ROOT` warns rather than refuses when that ancestor differs, so `create`, `inject`, `list`, and `prune-gitignore` are covered now: the first through `execScript`, the middle two through `prepare()`, and `list` through an explicit call of its own.

`canon tooling reference` is the one tooling verb still exempt, alongside `canon standards <name>`, and both for the same reason. `referenceRoots` and `standardRoots` put the caller's own working root ahead of the package corpus, so a caller standing in the checkout it means to read already reads that checkout. What survives there is narrower: a cwd one level short of the `tooling/` or `standards/` folder falls through to the package corpus with nothing said. Recorded rather than fixed, since the precedence those two resolvers implement was chosen on purpose. The `.claude/ARCHITECTURE.md` entry on the warning's reach carries the cross-domain accounting for every other verb.
...
Measured 2026-09-06 building `nextjs`: `create-next-app`'s current non-interactive flags are `--skip-install` and `--disable-git`, not the older `--no-install`/`--no-git`, and there is no `--turbopack` flag since Turbopack is the default bundler now. `--no-agents-md` suppresses the scaffold-time `AGENTS.md`/`CLAUDE.md` write entirely, a cleaner fix for the root-`CLAUDE.md` collision than deleting the files after the fact. `next typegen && tsc --noEmit` is required for the stack's `typecheck` script rather than bare `tsc --noEmit`, since the App Router's route-level types (`LayoutProps`, `PageProps`) generate into a gitignored `.next/types/` absent from a fresh checkout, caught by the Check phase. Running the scaffold in `/tmp` showed `create-next-app`'s default `.gitignore` already covers `.next/`, which is what let `nextjs` ship with no `[gitignore]` group at first. `nextjs` gained one anyway on 2026-09-06, naming `.next/` to match `astro`'s `[gitignore]` group, since a target whose own `.gitignore` never carried that scaffold default has nothing else declaring the entry. `eslint-plugin-check-file`'s `folder-naming-convention` was measured against the App Router's special folder syntax (`[id]`, `(group)`, `@slot`) and reported nothing, read at the time as needing no `src/app/**` exemption the way astro's `pages/**` needed one. That run was against a rule dormant across all three configs from the missing trailing slash, and the corrected 2026-09-07 measurement below finds `src/app/**` needs the identical off-block.

Measured 2026-09-06 against the shipped `tooling/astro/configs/eslint.config.js`, in an isolated scratch install rather than a committed devDependency. The first real ESLint run reported nothing for `check-file/filename-naming-convention` against a PascalCase `.astro` fixture (`MyComponent.astro`), confirming the config-reading finding that the option's `.astro` pattern sat inside a block scoped to `**/*.{ts,tsx,js,jsx}` and never reached it. A first narrowing attempt, `**/*.{ts,tsx,js,jsx}` matching the block's own selector exactly, turned out to newly enable the rule against `.js`/`.jsx` filenames, which neither `tooling/web/configs/eslint.config.js` (`**/!(App).{ts,tsx}`) nor `tooling/nextjs/configs/eslint.config.js` (`**/*.{ts,tsx}`) enforces despite selecting those extensions in the identical block, confirmed against `micromatch.isMatch` directly.

The operator reversed the narrow-versus-widen call once that finding was in hand. `web` and `nextjs` already gate their component filenames `KEBAB_CASE`, so narrowing left `astro` the one stack of three enforcing nothing, and Astro's own PascalCase convention lost out deliberately: a component's name in markup comes from the import binding rather than the filename, so kebab-case costs nothing at the call site. The shipped block widened its `files` selector to `**/*.{ts,tsx,js,jsx,astro}` and the rule's own pattern kept `.astro` (`**/*.{ts,tsx,astro}`), while `.js` and `.jsx` stayed out of the rule's own pattern, carrying the sibling-stack finding above forward unchanged. A second real ESLint run against the widened config confirmed `check-file/filename-naming-convention` now fires on `MyComponent.astro` and on `Thing.astro` inside a PascalCase-named folder holding only an `.astro` file (`PascalFolder/Thing.astro`), a kebab-case `.astro` fixture (`my-component.astro`) stays clean, and the bad-cased `.ts` fixture still fires. No parse error surfaced from folding `.astro` into the same block that runs `simple-import-sort` and `@typescript-eslint/no-unused-vars`, so the merge with `eslint-plugin-astro`'s own `.astro`-scoped block from `astro.configs.recommended` composes cleanly at this version pair.

`check-file/folder-naming-convention` turned out to reach no folder at all, `.astro`-only or otherwise, under any of the three configs measured. A PascalCase folder holding only an `.astro` file and a separate PascalCase folder holding a `.ts` file both linted clean, before and after the block widened. The rule's own implementation calls `micromatch.capture()` against its configured pattern to find the path segments to check, and the shipped pattern, `src/**/!(__tests__|pages)` with no trailing slash, returns `undefined` from that call for every directory tested, which the rule reads as nothing to check and skips. Adding a trailing slash (`src/**/!(__tests__|pages)/`) restores a real capture, confirmed directly against `micromatch` outside ESLint. The rule is inert for the whole stack today, not narrowly for `.astro`, and fixing the pattern is a separable change this measurement does not take.

ESLint 10.10.0, the latest release at measurement time, crashes on load against `typescript-eslint@8.69.0`'s bundled `@typescript-eslint/utils`, throwing `Class extends value undefined is not a constructor` from its `LegacyESLint` shim, because ESLint 10 dropped `LegacyESLint` from `eslint/use-at-your-own-risk`. Pinning `eslint@9` in the scratch install avoided the crash and produced the runs above.

Measured 2026-09-07: the trailing-slash fix restores a live `check-file/folder-naming-convention` against all three shipped configs, confirmed by a real ESLint run against fixtures shaped like each stack's own routing convention. `web` lints clean off the slash alone, since neither `web` nor `vite-react` carries a bracket-, paren-, or at-sign-named routing folder. `astro`'s `src/pages/[id]/` and `src/pages/blog/[...slug]/` both fail `KEBAB_CASE` once the pattern captures anything, so `pages/**` needs its own off-block rather than the segment-only exemption the shipped pattern used to carry. `nextjs`'s `[id]`, `(group)`, and `@slot` folders under `src/app/**` fail the same way, so that stack's existing `src/app/**` override block gains `check-file/folder-naming-convention: 'off'` beside the `react-refresh` exemption already there.
