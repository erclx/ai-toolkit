...
- `scripts/screenshot.sh` reports exit 1 on an `astro` target even when both PNGs capture correctly. `astro preview` forks a detached background process of its own, so the wrapper's backgrounded `bun run preview &` job has already exited by the time the script's `EXIT` trap runs `kill "$PREVIEW_PID"`, and that `kill` failure trips `set -e` inside the trap, overwriting the script's already-successful exit status. Reproduced in isolation and against `canon tooling verify astro --keep` twice. Measured 2026-09-05, unfixed.
