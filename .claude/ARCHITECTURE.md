...
A census run over all 27 standards confirms that reasoning and bounds it. The fan-out is a destination with a ceiling rather than a mechanism to reject, affordable while a consumer set stays at roughly a handful and replaced by a verb where it does not, which is what routes `markdown.md` at 21 bodies and `slug.md` at 13 to the CLI while seven standards keep the fan-out. What decides each one is the branch its own `## Scope` already declares, since a document-type standard names the paths a governance rule can glob and an attribute standard resolves to `*` and has no document to glob at all. The per-standard record and the empty drop bucket sit in `.claude/context/standards/destinations.md`. Measured at `bd2be81a` on 2026-08-20.
