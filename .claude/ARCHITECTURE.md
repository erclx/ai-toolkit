...
What the census got wrong is what sits above the ceiling. It recorded a verb there, on the reading that a value every caller reads is a value a verb should return, and all three verbs it named were withdrawn on 2026-08-20 without one being built. A verb collapses a duplication, and `slug.md`, `publish.md`, and `versioning.md` have none to collapse, since every citing body names the standard and states its own response rather than restating the rule. A standard over the ceiling therefore stays in the flat corpus reached by citation, which is not a home outside the corpus and leaves those three unrouted when the folder goes. Measured at `653bbb15` on 2026-08-20.
