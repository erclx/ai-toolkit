...
A census over all 27 standards confirms that reasoning and bounds it: the fan-out is a destination with a ceiling rather than a mechanism to reject, affordable while a consumer set stays at roughly a handful and replaced by a verb where it does not. What routes each one is the branch its own `## Scope` already declares, so the test reads a field the corpus maintains rather than a judgment the census makes, and the per-standard record with the empty drop bucket sits in `.claude/context/standards/destinations.md`. Measured at `bd2be81a` on 2026-08-20.
