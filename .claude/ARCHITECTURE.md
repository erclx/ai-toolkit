...
A session loads a skill body once and keeps it, and re-invoking the skill reports the instructions unchanged rather than re-reading the file. Two routes reach that. A session outliving a merge someone else made goes on applying what it loaded, and a compaction carries the held copy forward with the summary, which makes that exposure the age of the oldest load rather than the age of the session. The second needs no age and is the ordinary ship shape here, since a branch touching `claude-docs`, `git-stage`, or `git-pr` runs those skills later in the chain that edited them.
