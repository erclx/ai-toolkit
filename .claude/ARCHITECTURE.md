...
- Verification anchors on this file have no writer, so an anchor goes stale in the way it exists to surface. `claude-docs` amends a decision's reasoning here without anchoring it, breaking the rule on every run that touches this file, and it is also where the sweep belongs, since it already reads diagram frontmatter for staleness.
