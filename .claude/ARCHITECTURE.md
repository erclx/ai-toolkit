...
- Skills and the CLI ship at two speeds. A skill merged to `main` reaches a `--plugin-dir` session immediately, while the CLI reaches a user only once a release cuts a tag and the publish job lands it on the registry. A skill calling a verb or flag that has not been published yet fails in a target, and nothing detects it.
  ...
  - A missing subcommand under a command that exists is the quieter half of the same gap. Commander answers that one with `unknown command` on exit code 0, so a body branching on the exit reads success, receives no record, and routes on nothing rather than refusing. Pointing the `claude-docs` plans sweep at `aitk tasks plan-citations` was measured against the `claude:docs` `board-sweep` arm and archived neither plan, created no `.claude/plans-archive/`, and raised no error. The verb ships and the body keeps stating the rule until a release carries it, which is a duplication taken deliberately over a sweep that reports a clean pass having done nothing.
