...
- Verification anchors on this file have no writer. A decision is anchored by whoever writes or amends it, and nothing sweeps the record for a claim whose number has moved, so an anchor goes stale in the way it exists to surface. `claude-docs` amends a decision's reasoning here without anchoring it, so it breaks the rule on every run that touches this file, and it is also the natural home for the sweep, since it already reads diagram frontmatter for staleness.
