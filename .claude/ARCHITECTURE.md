...
A census over all 27 standards confirms that reasoning and bounds it: the fan-out is a destination with a ceiling rather than a mechanism to reject, affordable while a consumer set stays at roughly a handful. What routes each one is the branch its own `## Scope` already declares, so the test reads a field the corpus maintains rather than a judgment the census makes, and the per-standard record with the empty drop bucket sits in `.claude/context/standards/destinations.md`. Measured at `bd2be81a` on 2026-08-20.

What the census got wrong is what sits above the ceiling. It recorded a verb there, on the reading that a value every caller reads is a value a verb should return, and both verbs it named to write were withdrawn on 2026-08-20 without either being built. `markdown.md` is the CLI destination that arrived, and what earned it is a closed list a check already walks. A verb collapses a duplication, and `slug.md` and `publish.md` have none, since every citing body names the standard and then states its own response rather than restating the rule.

`versioning.md` was withdrawn the same day and answers to neither argument, having been routed through the fan-out rather than to a verb. What stops it is that two of its readers are standards depending on it in-body while a bundled standard never installs, so the move would point both at a file no target holds. Three standards therefore stay in the flat corpus reached by citation, which is not a home outside the corpus and leaves all three unrouted when the folder goes. Measured at `653bbb15` on 2026-08-20.

A machine-parsed standard is permanently exempt from any design inlining a standard into the rule that cites it, because a rule restating the list a parser reads is two sources for one list. The set is empty: `markdown.md` and the retired `prose.md` were the two members and neither is parsed any more, since the three ban sets and the six structural checkpoints now ship as data in `src/markdown/`. Stating the rule rather than the pair is what leaves it standing for a parser that arrives later. Measured at `60fc97bf` on 2026-08-19, and the destination census re-read the corpus on 2026-08-20 and admitted no new member, so the rule still stands over nothing.
...
A session loads a skill body once and keeps it, and re-invoking the skill does not re-read the file, so an edit made this session is not picked up. Two routes reach that. A session outliving a merge someone else made goes on applying what it loaded, and a compaction carries the held copy forward with the summary, which makes that exposure the age of the oldest load rather than the age of the session. The second needs no age and is the ordinary ship shape here, since a branch touching `claude-docs`, `git-stage`, or `git-pr` runs those skills later in the chain that edited them.

Which shape the staleness takes decides whether anyone notices, and neither route predicts it. A held body naming a file the branch deleted fails loudly by having nothing to read, which is what one commit moving sixteen shipped bodies met on the six it then invoked. One naming a file that still exists while saying something different resolves and reads current, so the session applies the superseded rule and reports success. A review body held from before `#1012` posted three passes under the retired heading rule, each of which looked correct.

`aitk claude skills drift <ref>` names which shipped bodies moved between a ref and `HEAD`, and `standards/session.md` runs it while a session map is written. Reading history is what keeps it off the second route, since a chain commits at `git-stage` after it has already invoked the bodies it edited. Comparing the bytes a session holds was the obvious design and cannot be built, because a session has no way to read its own loaded body back. The substitute costs exactness, since it reports a file moving rather than a held copy differing and over-reports on an old ref, which is the safe direction when confirming a name costs one read and the failure being answered is silence.

Instructing a session to re-invoke the skill was the cheap remedy and it fixes nothing, which is what rules out a one-line answer here. Adding a re-read step to the ship chain is that remedy aimed at the second route, and it is declined on cost, since it fires on every run to answer a case seen twice and one of those failed loudly. The instruction sits in `governance/rules/claude/570-skill.md` instead, which fires on the body being edited at the cost of splitting the remedy across two carriers, neither of which is a check. Measured at `34ce48e6` on 2026-08-20.

### A standard governs an artifact's shape and the skill driving it governs the procedure

`standards/teach.md` fixes the layout, the ordinal naming, the frontmatter, and the mission and learning-record formats of a learning workspace. The glossary shape sits apart from it, because that shape travels with the file wherever a promotion lands it and a workspace is one of the places it sits rather than the only one. The pedagogy that decides what to teach next sits in `claude/skills/claude-teach/references/`. The split is the one `standards/intake.md` already states about itself, applied to a surface built after the rule was written rather than before.

The glossary shape moved to `standards/bundled/glossary.md` on 2026-08-20 and now fans out beside the pedagogy it was separated from, which is the same reasoning carried one step further rather than reversed. A promoted glossary lands wherever its destination surface puts it, so no glob covers the set and one skill drives every move, which is the bundled test exactly. What it costs is that a target installing standards no longer receives the file, so `standards/teach.md` and `561-teach.md` name the skill rather than a path, and a project holding governance without the plugin is told to report the gap rather than proceed. Measured at `653bbb15` on 2026-08-20.
...
Moving the pen to match the sentence was the alternative and it is declined on that middle number. The 44 state an approach governing how the agent works and no context entry owns any of them, so a pass retiring every `project` entry would delete rules the folder exists to hold. The ownership test survives untouched because it was never the part that failed, and `standards/memory.md` already admitted `project` as a first-class type in the table two sections above the clause that contradicted it. The pen and the rule agree once the 98 fold into their entries and retire, which is the act this record's own reasoning waited on, and it landed as one pull request rather than the twelve the plan proposed, one per destination domain. What decides the split is that the reviewable unit is a passage already written for its destination rather than a judgment, and every judgment had been reviewed when the classification was made. The fold retired 98 entries into the archive, which now holds 100 against the 2 an earlier pass put there, so what the pen keeps is what the ownership test admits. Measured at `c7e92612` on 2026-08-20.
