...
- Verification anchors on this file have no writer. A decision is anchored by whoever writes or amends it, and nothing sweeps the record for a claim whose number has moved, so an anchor goes stale in the way it exists to surface. `claude-docs` already sweeps diagram frontmatter and is the natural home for the pass.
