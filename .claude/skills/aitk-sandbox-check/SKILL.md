...
- For plugin skills, use `<skill-name>` as the item path and append `# /<skill-name>` as the invocation hint. For scripts under `scripts/`, use the path relative to the repo root (`scripts/core/verify.sh`) and omit the hint. For `src/**` items, append `# Closest e2e: bun run check:install` as the hint.
