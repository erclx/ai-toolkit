...
- For `claude/` scenarios, default to `SANDBOX_INJECT_SEEDS="true"`. See the rule and its two exceptions in `.claude/context/sandbox.md`.
...
- To test a skill non-interactively, run `scripts/sandbox/run.sh <cat:cmd> "/aitk:<skill>" [scenario]`. It provisions the scenario, drives the skill through `claude -p`, and returns a JSON envelope (`is_error`, `result`, `num_turns`, `total_cost_usd`). Judge the run from the envelope instead of opening an interactive session. See `.claude/context/sandbox.md`.
...
- `github.com/erclx/toolkit-sandbox` is a pre-authorized destructive-ops playground. Sandbox scenarios that force-push to its `main` or delete `chore/aitk-sync*` and `chore/toolkit-sync*` remote branches run without confirmation. The second pattern covers branches generated before the sync prefix rename and drops out once the sandbox remote carries none. Treat any other `erclx/*` repo as production where destructive ops still need explicit approval.
