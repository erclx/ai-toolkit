...
- Governance rules and stacks, which are a separate catalog reaching a target through a separate install: `aitk-governance`
- The conventions a stack's `reference.md` follows, which the internal tooling-reference standard states
- The content of the seeded Claude planning docs, which `aitk-claude` owns. This skill owns how a seed layers, merges, and survives sync.
