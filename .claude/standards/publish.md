...
Governs an attribute rather than a document type: the scan an author runs against finished text on its way out, which applies to any text leaving through a channel no automated check covers. It also fixes the responses available when a source the scan reaches for cannot be read.
