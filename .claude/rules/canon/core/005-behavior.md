...
- Rewrite the `- Suggested:` line rather than the blank `- Answer:` slot when execution deviates from a plan question left for the operator's call, per the plan standard, which your toolkit resolves by name.
