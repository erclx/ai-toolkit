...
- Follow `standards/teach.md` inside the aitk plugin for the folder layout, ordinal naming, frontmatter, and the mission and learning-record formats. It is the single source.
...
- Report it rather than proceeding silently when either file does not resolve. Both ship with the plugin and this rule ships with the CLI, so a project that installed governance alone does not have them.
