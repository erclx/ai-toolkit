...
| Shipped skill bodies (`claude/skills/`)    | no           | no                                  |
...
- A phase label in a context entry or a standard is permitted only when its substance is restated inline beside it. A bare label carrying nothing beside it is forbidden, since a reader without the board has nothing to resolve it against.
- A shipped skill body carries no phase label, substance restated or not. It installs into a target project's plugin cache and is read there by a session holding no checkout of this repository at all, so the label resolves to nothing whatever the surrounding sentence says.
