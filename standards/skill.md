...
- Choosing which to author: content a target project authors on its own is a standard at `.claude/standards/X.md`. Content only one skill copies verbatim, such as a code or config template, is a skill reference under `references/` and follows `prose.md` like any other prose doc. When unsure, default to a standard, since a shared reference is harder to unbundle later than a standard is to reference.
