...
- Past roughly 40 rendered lines with no heading of any level breaking them, add a subheading at the seam. Measure the longest such run rather than everything under one `##`, wrap each source line at 80 columns to get the height it renders at, and exclude fenced code blocks. Source lines undercount an entry authored one line per bullet, where a block of fifteen paragraph-bullets renders past sixty. The number is a checkpoint like the 150 above, not a cap.
