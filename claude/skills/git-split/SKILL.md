...
Before running them, run the scan in `.claude/standards/pre-publish.md` against every title and body, or `${CLAUDE_SKILL_DIR}/../../standards/pre-publish.md` when the project does not have it. Each body stages under `.claude/.tmp/` and leaves through `gh`, so the hook sees neither and this scan is the only gate.
