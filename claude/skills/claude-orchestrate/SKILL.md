...
first, and merge a branch touching `CLAUDE.md`, a Claude context entry, or a
regenerated `index.md` last. Have every sibling rebase on the new `main` before
the next merge. Assign a distinct port per track when two workers run a server,
since each session spawns its own process.
