...
- STALLED: read the last pass and report what it carried. A pass carrying a critical or a should-fix is waiting on the worker who holds it, so confirm the dispatch went out and send it if it did not. A pass carrying only minors is under the wrong heading and owes nobody, so report that and stop rather than dispatching on them. Do not re-run a review to correct the heading, since a pass on an unchanged head with no response behind it stops by design.
...
`STALLED` is the one state the script derives from a heading rather than from a commit or a count, since `claude-pr-review` posts the open heading only for a pass carrying a critical or a should-fix. It fires once per entry into the state, on the second consecutive run where the open pass covers the head and nothing has followed it, and it fires again after any commit or reply that resets the thread. A worker slower than one poll interval reads the same way, which is why the rule above asks for a confirmation rather than treating the report as a verdict.
