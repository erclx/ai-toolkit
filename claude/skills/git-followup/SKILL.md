...
6. If the count is above zero, the followup addresses review feedback: post a one-line summary of the fix with `gh pr comment --body`. If it is zero, run `gh pr view --json url,title,body` and update the body with `gh pr edit --body` when the new commit changes scope, and the title with `gh pr edit --title` when the scope shifted enough to make it inaccurate.
