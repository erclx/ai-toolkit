...
If nothing is wrong, say so: `✅ No observations.`
...
Write the full audit, including the summary line and per-surface blocks, to `.claude/review/ux-audit-<slug>.md` from the project root. Create the directory if it does not exist. Always overwrite. Output `📝 Wrote .claude/review/ux-audit-<slug>.md` after the chat report.
