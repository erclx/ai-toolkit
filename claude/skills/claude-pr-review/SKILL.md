...
A non-empty result carries the comment id Step 4 needs for the third filename segment. Read that comment for what the worker changed or accepted, or for a finding it raises under `## Post-review findings`, and treat an accepted finding as closed rather than restating it. This is the entire read on a repeated head, since the empty range above has nothing in it to say whether a prior finding landed. Skip the diff and file reads below.
