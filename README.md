...
`aitk init` installs base tooling configs, Claude seeds, snippets, and a wiki stub in one pass. Pass `--stack <name>` to also install governance rules for a framework stack into `.claude/rules/`. Run `aitk tooling list --json` to see the catalog.
