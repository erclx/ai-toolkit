...
Bootstrap a new project: `aitk init` installs base configs, Claude workflow, governance, snippets, and wiki in one command. The `standards` domain is offered interactively as an opt-in. Pass flags to run non-interactively: `--stack <name>`, `--add <rules>`, `--snippets <cat>`, `--with standards`, `--skip wiki`. Governance is skipped when `--stack` is absent. The `setup-init` skill resolves these from project detection and runs the chain in one shot.
