...
- `--skip governance`: leave `.claude/rules/` empty, so no coding standard loads on a file match. Standards still install, and the installed copy is an editable seed rather than the file anything reads, since a rule and a skill both name the corpus inside the plugin. The preview names any `--add` rules the skip drops, and the run prints the `aitk gov install <stack> <path>` command to add rules afterward, carrying those extras so one paste restores what the skip declined.
- `--skip standards`: leave standards out. The governance rules are unaffected, since each authority line names the corpus inside the aitk plugin and adds a bullet telling a session to report the gap when that file does not resolve. Toolkit skills are unaffected for the same reason, each naming the copy in its own plugin root. `aitk standards <name>` searches the corpus inside the CLI's own package behind both project roots, so it prints a standard in a project that skipped the install, and `aitk markdown audit` needs no standard at all, its ban sets and checkpoints shipping with the package as data. What the skip costs is the editable copy rather than the enforcement.
