...
To run several worktrees as a coordinated flow rather than ad hoc, assert the orchestrator role in one warm session with `aitk:claude-orchestrate`. It plans each feature, refills the ready queue so a free worker never waits, and reviews each worker's PR with `aitk:claude-pr-review`, while workers address the posted findings with `aitk:claude-address-review`. It owns the roadmap via `aitk:claude-roadmap` while a scope exists to sequence, and reads `.claude/tasks/priority.md` for execution order once that scope has shipped and later work arrives as discrete items. The human launches workers and merges. See [operating model](../wiki/operating-model.md) for the full loop.
