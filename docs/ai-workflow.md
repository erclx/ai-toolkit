...
An execution that picks other than the suggestion rewrites the `- Suggested:` line to what it picked, names the measurement that moved it, and leaves the slot blank. That measurement is how a reader of the archived plan tells an override from a suggestion the execution accepted. The same deviation takes one line in the open task's `## Findings`, which is the register that survives the plan being archived. A deviation from a question somebody already answered goes back to whoever answered it instead, since a filled slot is a decision already made.
