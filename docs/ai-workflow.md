...
Review is skipped when the diff is prose that only informs: every changed file matches `*.md` or `*.txt`, and none sits under a behavior path. Behavior paths cover skills, rules, and `tooling/` in both the authoring and the installed spelling, so the list matches whether a repository authors those surfaces or consumed them from the toolkit. Standards and snippets carry the authoring spelling alone, since each reaches a session by resolution rather than by an install, and root `CLAUDE.md` is named as a file because a path prefix reaches nothing sitting in no folder.
