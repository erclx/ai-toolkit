...
Line counts are text-only. A file that reads as binary is counted toward the file total and its extension's file count, and left out of every line count, and the record's `skipped` field states the gap rather than leaving a reader to infer it from the difference between the file count and the summed extension line counts.
