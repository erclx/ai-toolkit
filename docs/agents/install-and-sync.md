...
destination shape from the target's own installed copy, and proposes the split
without writing. Where a folder has no governing standard, or the target never
installed the one it has, the entry earns a named refusal rather than a shape the
