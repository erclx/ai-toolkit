...
Append `<!-- audit-ignore-citations -->` to the source line in either case. The marker suppresses citation checking for its own line only. Where a marker would ship into a file the target reads, such as a seed or a plugin skill body, reword the line to drop the path instead.
