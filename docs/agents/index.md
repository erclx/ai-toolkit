...
- [Rule citations](rule-citations.md): Resolving every path a governance rule cites, the three forms a citation is written in, the shapes that look like citations and are not, the two classes where an absent path is correct, the blind spots it cannot reach, and why this one gates
