...
The required-section check reports a folder no entry of which declares `## Overview` and `## Layout`, the two sections `.claude/standards/context.md` marks required. The list is held in code beside the numeric checkpoints rather than parsed out of the standard, so it fails on a defect rather than on a rewrite of the wording around it. A heading at any level counts, because a domain that split into a folder carries its overview in a sibling named `overview.md` where the section is the `#` title and an `##` beneath it would repeat the filename. Matching exactly is what keeps `## Layout catalog` from satisfying `Layout`.

The judgment is per folder rather than per file, since any one sibling answers for the rest. Demanding both of every entry would report every other child of a split domain, which teaches a reader to skip the whole check. It reports rather than gates, the closer call because a missing section reads more like a fact than the other judgments. What settles it is that the standard sanctions omitting `## Layout` from a domain owning no paths in the repo, and no measure separates that from an entry that forgot it. The JSON record carries the findings as `missingSections` and the list as `checkpoints.requiredSections`.
